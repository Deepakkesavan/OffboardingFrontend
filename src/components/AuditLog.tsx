...
type ActionVariant = 'info' | 'success' | 'danger' | 'primary' | 'warning' | 'neutral';
...
const VARIANT_COLORS: Record<ActionVariant, string> = {
  info:    'var(--clr-info)',
...
export default function AuditLog({ recordId }: AuditLogProps) {
  const { data: logs = [], isLoading } = useFetch<AuditLogEntry[]>(
  ...
  return (
    <div className="audit-wrap card">
      <div className="card-header">
        ...
        <span style={{ fontSize: '.8rem', color: 'var(--clr-text-muted)' }}>
          {logs.length} events
        </span>
      </div>
      ...
      {logs.length === 0 ? (
        ...
        <div className="audit-timeline">
          {[...logs].reverse().map((log, idx) => {
            const meta  = ACTION_LABELS[log.action] ?? { label: log.action, icon: '•', variant: 'neutral' as ActionVariant };
