...
import type { StageScreenProps, ManagerReviewPayload, ExitInterviewPayload } from '../../types';

type InterviewMode = 'In-Person' | 'Video Call' | 'Phone';
type ApprovalDecision = 'approved' | 'rejected';

const INTERVIEW_MODES: InterviewMode[] = ['In-Person', 'Video Call', 'Phone'];

export default function ManagerApproval({ record, stageData, onStageChange }: StageScreenProps) {
  const managerStage = stageData.find((s) => s.stageType === STAGES.MANAGER_REVIEW);
  const isCompleted  = !!managerStage?.completedAt;
  const saved        = (managerStage?.payload ?? {}) as Partial<ManagerReviewPayload>;

  const exitStage   = stageData.find((s) => s.stageType === STAGES.EXIT_INTERVIEW);
  const exitPayload = (exitStage?.payload ?? {}) as Partial<ExitInterviewPayload>;

  const [form, setForm] = useState<ManagerReviewPayload>({
    ...
    approvalDecision: saved.approvalDecision ?? undefined,
    rejectionReason:  saved.rejectionReason  ?? '',
  });

  const onChange =
    <K extends keyof ManagerReviewPayload>(field: K) =>
    (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) =>
      setForm((f) => ({ ...f, [field]: e.target.value }));

  const { mutate, isPending } = useMutation<void, ApprovalDecision>(
    async (decision) => {
      const now     = new Date().toISOString();
      const payload: ManagerReviewPayload = { ...form, approvalDecision: decision };
    ...
    { onSuccess: () => onStageChange(STAGES.HR_INITIATION) },
  );
      ...
      <div className="form-grid cols-1">
        <div className="form-group">
          ...
          <textarea
            ...
            onChange={onChange('managerNotes')}
        ...
        <div className="form-group">
          ...
          <textarea
            value={form.handoverPlan}
            onChange={onChange('handoverPlan')}
            readOnly={isCompleted}
            rows={3}
        ...
        {isRejected && (
          <div className="form-group">
            ...
            <textarea
              ...
              onChange={onChange('rejectionReason')}
              readOnly={isCompleted}
              rows={2}
            />
          </div>
