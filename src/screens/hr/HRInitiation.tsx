...
import type { StageScreenProps, HRInitiationPayload, ManagerReviewPayload, ClearanceDepartment } from '../../types';

const CLEARANCE_DEPTS: ClearanceDepartment[] = ['IT', 'Finance', 'Admin', 'Security', 'HR'];

export default function HRInitiation({ record, stageData, onStageChange }: StageScreenProps) {
  const hrStage     = stageData.find((s) => s.stageType === STAGES.HR_INITIATION);
  const isCompleted = !!hrStage?.completedAt;
  const saved       = (hrStage?.payload ?? {}) as Partial<HRInitiationPayload>;

  const managerStage   = stageData.find((s) => s.stageType === STAGES.MANAGER_REVIEW);
  const managerPayload = (managerStage?.payload ?? {}) as Partial<ManagerReviewPayload>;

  const [form, setForm] = useState<HRInitiationPayload>({
  ...
  const onChange =
    <K extends keyof HRInitiationPayload>(field: K) =>
  ...
  const { mutate, isPending } = useMutation(
    async () => {
      ...
      onStageChange(STAGES.CLEARANCES);
      return STAGES.CLEARANCES; 
    },
    ...
    <div className="card">
      ...
      <div style={{ background: 'var(--clr-surface-2)', borderRadius: 'var(--radius-md)', padding: '16px', marginBottom: '24px', border: '1px solid var(--clr-border)' }}>
        ...
        <div className="form-grid">
          ...
          <div className="form-group">
            ...
            <input
              value={managerPayload.approvalDecision ?? '—'}
              readOnly
      ...
      <div className="form-grid">
        ...
        <div className="form-group">
          ...
          <input
            value={form.assetReturn}
            onChange={onChange('assetReturn')}
            readOnly={isCompleted}
        ...
        <div className="form-group">
          ...
          <input
            ...
            onChange={onChange('systemAccess')}
            readOnly={isCompleted}
        ...
        <div className="form-group">
          ...
          <input
            ...
            onChange={onChange('payrollNotes')}
            readOnly={isCompleted}
        ...
        <div className="form-group full-width">
          ...
          <textarea
            value={form.offboardingPlan}
            onChange={onChange('offboardingPlan')}
            readOnly={isCompleted}
            rows={4}
            placeholder="Detailed offboarding plan and instructions for each department..."
          />
        </div>
      </div>
