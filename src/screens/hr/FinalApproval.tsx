...
import type { StageScreenProps, FinalApprovalPayload } from '../../types';
...
function calculate90Days(submittedAt: string | undefined): NoticePeriodResult {
  ...
  const daysRemaining = Math.max(
    0,
    Math.ceil((noticeEnd.getTime() - Date.now()) / (1000 * 60 * 60 * 24)),
  );
  return { noticeEndDate: noticeEnd.toISOString().split('T')[0], daysRemaining };
}

export default function FinalApproval({ record, stageData, onStageChange }: StageScreenProps) {
  const finalStage  = stageData.find((s) => s.stageType === STAGES.FINAL_APPROVAL);
  const isCompleted = !!finalStage?.completedAt;
  const saved       = (finalStage?.payload ?? {}) as Partial<FinalApprovalPayload>;

  const { noticeEndDate, daysRemaining } = calculate90Days(record.submittedAt);

  const [form, setForm] = useState<FinalApprovalPayload>({
  ...
  const onChange =
    <K extends keyof FinalApprovalPayload>(f: K) =>
  ...
  const { mutate, isPending } = useMutation(
    async () => {
      ...
      onStageChange(STAGES.COMPLETED);
        ...
        <div className="form-grid">
          ...
          <div className="form-group full-width">
            ...
            <textarea
              ...
              onChange={onChange('resignationLetter')}
              readOnly={isCompleted}
              rows={5}
              placeholder="I hereby resign from my position effective..."
            />
          </div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div style={{ background: 'var(--clr-primary-light)', border: '1px solid #bfdbfe', borderRadius: 'var(--radius-md)', padding: '16px', marginBottom: '24px', display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '1rem', fontWeight: 600, marginTop: '4px' }}>
              {noticeEndDate ? new Date(noticeEndDate).toLocaleDateString('en-IN') : '—'}
            </div>
          </div>
          <div>
            ...
            <div style={{ fontSize: '1rem', fontWeight: 600, marginTop: '4px', color: (daysRemaining ?? 0) <= 10 ? 'var(--clr-danger)' : 'var(--clr-text-primary)' }}>
              {daysRemaining} days
            </div>
          </div>
        </div>
        ...
        <div className="form-grid" style={{ marginBottom: '24px' }}>
          {(['gratuityApplicable', 'pfTransfer', 'exitBonus'] as const).map((field) => (
            <div className="form-group" key={field}>
              <label>
                {field === 'gratuityApplicable' ? 'Gratuity Applicable'
                  : field === 'pfTransfer' ? 'PF Transfer Required'
                  : 'Exit Bonus Applicable'}
              </label>
              <select value={form[field]} onChange={onChange(field)} disabled={isCompleted}>
        ...
        <div className="form-grid cols-1">
          ...
          <div className="form-group">
            ...
            <textarea value={form.hrComments} onChange={onChange('hrComments')} readOnly={isCompleted} rows={3} placeholder="Any final remarks or instructions..." />
          </div>
        </div>
