...
import type { OffboardingRecord, Role, Stage, StageData } from '@/types';
...
function StageScreen({ record, stageData, activeRole, onStageChange }: {
  ...
  const allowedRoles = STAGE_ROLE_MAP[stage as Stage] || [];
  const hasAccess    = allowedRoles.includes(activeRole);
  const props        = { record, stageData, activeRole, onStageChange };
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 'var(--sp-lg)' }}>
        <ExitClearanceForm record={record} />
      </div>
    </div>
  );
}
