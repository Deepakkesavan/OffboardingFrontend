...
function isFinanceUnlocked(endDate: string | null | undefined): boolean {
  ...
  return (new Date(endDate).getTime() - Date.now()) / (1000 * 60 * 60 * 24) <= 2;
}

interface StakeholderClearanceProps {
  record: OffboardingRecord;
}

export default function StakeholderClearance({ record }: StakeholderClearanceProps) {
  const [confirmOpen, setConfirmOpen] = useState(false);
  const [activeClear, setActiveClear] = useState<Clearance | null>(null);
  ...
  const handleClearClick = (clearance: Clearance) => {
              ...
              >
                ...
                {c.isCleared && (
                  <div className="sc-cleared-meta">
                    ✓ Cleared by <strong>{c.clearedBy}</strong> on{' '}
                    {new Date(c.clearedAt!).toLocaleDateString('en-IN')}
