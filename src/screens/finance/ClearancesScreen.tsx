...
import {
  getClearances, updateClearance, updateRecord, createStageData,
  addAuditEntry, updateStageData,
} from '../../api/offboardingApi';
import { STAGES } from '../../store/offboardingStore';
import useStore from '../../store/offboardingStore';
import type { StageScreenProps, Clearance, ClearanceDepartment } from '../../types';
...
function isFinanceUnlocked(endDate: string | null | undefined): boolean {
...
function ClearanceRow({
  ...
  return (
    <div style={{
      ...
      padding: '16px',
      background: clearance.isCleared
        ? 'var(--clr-success-bg)'
        : locked
        ? 'var(--clr-surface-2)'
        : 'var(--clr-surface)',
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span style={{ fontSize: '1.4rem' }}>
            {DEPT_ICONS[clearance.department as ClearanceDepartment] ?? '📋'}
          </span>
          <div>
            ...
            {locked && (
              <div style={{ fontSize: '.8rem', color: 'var(--clr-warning)' }}>🔒 Unlocks at T-2</div>
            )}
            {clearance.notes && (
              <div style={{ fontSize: '.8rem', color: 'var(--clr-text-muted)', marginTop: '2px' }}>
                {clearance.notes}
              </div>
            )}
          </div>
...
export default function ClearancesScreen({ record, stageData, onStageChange }: StageScreenProps) {
  ...
  } = useFetch<Clearance[]>(() => getClearances(record.id), [record.id]);
  ...
  const { mutate: markCleared, isPending: isClearing } = useMutation<void, { clearance: Clearance; notes: string }>(
  ...
  const { mutate: advance, isPending: isAdvancing } = useMutation(
    async () => {
      ...
      onStageChange(STAGES.FINAL_APPROVAL);
      return STAGES.FINAL_APPROVAL;
    },
    ...
    <div>
      {!financeUnlocked && (
        <div className="alert alert-warning">
          ...
          ({record.endDate ? new Date(record.endDate).toLocaleDateString('en-IN') : '—'}).
        </div>
      )}
